...
layui.define(['laytpl', 'layer', 'table', 'form'], function (exports) {
  var $ = layui.jquery,
    laytpl = layui.laytpl,
    layer = layui.layer,
    setter = layui.setter,
   ...
   */
  view.table = function (options) {

    table.render({
      elem: options.elem,
      url: setter.request.host + options.url,
      where: options.where || {},
  ...
  Class.prototype.render = function (views, params) {
    ...
    $.ajax({
      ...
      error: function (e) {
        ...
        if (e.status === 404) {
          that.render('system/404');
        } else {
          that.render('system/error');
        }
